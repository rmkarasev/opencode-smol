...
1. Installs plugin packages automatically at startup
2. Caches pinned versions until user changes config
3. For unpinned plugins, resolves `latest` and caches actual version
4. Does **not** rely on plugin package scripts during install

This means the README SHOULD NOT include `npm install` instructions - just tell users to add the plugin to their config. Because package scripts are not guaranteed to run during plugin installation, published plugins MUST ship prebuilt output in the npm tarball.
...
2. **package.json** (replace placeholders with user's answers):
   ...
   Notes:
    - SHOULD align `@opencode-ai/plugin` with the minimum OpenCode version you support (for example `^1.3.0`)
    - MUST publish built `dist/` output; do not depend on `postinstall` or `prepare` hooks to generate runtime files
