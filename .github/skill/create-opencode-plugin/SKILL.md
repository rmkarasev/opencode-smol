...
description: |-
  Create OpenCode plugins using the @opencode-ai/plugin SDK. Use for building custom tools, event hooks, auth providers, or tool execution interception. Use proactively when developing new plugins in .opencode/plugins/ or ~/.config/opencode/plugins/.
  Examples:
...
```bash
bun run .github/skill/create-opencode-plugin/scripts/extract-plugin-api.ts --workspace /absolute/path/to/opencode
```
...
| Scope   | Path                                        | Use Case                   |
| ------- | ------------------------------------------- | -------------------------- |
| Project | `.opencode/plugins/<name>.ts`               | Team-shared, repo-specific |
| Global  | `~/.config/opencode/plugins/<name>.ts`      | Personal, all projects     |
...
For complex plugins, use a thin auto-loaded entry file plus a modular source directory:

```
.opencode/plugins/my-plugin.ts   # Thin entry file auto-loaded by OpenCode
plugins-src/my-plugin/
├── index.ts                     # Real plugin implementation
