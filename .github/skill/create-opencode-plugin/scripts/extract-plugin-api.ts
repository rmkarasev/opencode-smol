 ...
 *
 * Usage: bun run .github/skill/create-opencode-plugin/scripts/extract-plugin-api.ts [--workspace /path/to/opencode]
 */
...
async function extractAuthHook(): Promise<string> {
  ...
  const authResultMatch =
    content.match(/export type AuthOAuthResult[\s\S]*?\n\)\n/m) ??
    content.match(/export type AuthOuathResult[\s\S]*?\n\)\n/m)
...
- \`tool.definition\`: Modify the definition exposed to the model
...
- \`provider\`: Extend provider/model definitions
    ...
    tool: {
      myTool: tool({
        ...
        async execute(args, context) {
          ...
          // context includes sessionID, messageID, agent, directory, worktree, abort, metadata(), ask()
